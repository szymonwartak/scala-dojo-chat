...
object ChatServer extends LiftActor with ListenerManager {
  private var msgs = Vector[ChatItem]{ChatItem("moderator", "Welcome")} // private state
  ...
  override def lowPriority = {
    case ChatItem(user, message) => msgs :+= ChatItem(user, message); updateListeners()
  }
}